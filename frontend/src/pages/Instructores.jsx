...
const Instructores = () => {
  ...
  const resetForm = () => {
    setFormData({ nombre_completo: '', especialidad: '', contacto: '', id_usuario: 1, estatus: 'Activo' });
    setEditMode(false);
    setCurrentId(null);
  };
    ...
    <div className="p-6 bg-[#0B0E11] min-h-screen text-white">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div className="bg-[#161B22] p-4 rounded-xl border border-gray-800">
          <p className="text-gray-400 text-xs uppercase font-semibold">Total Instructores</p>
        ...
        <div className="bg-[#161B22] p-4 rounded-xl border border-gray-800 border-l-4 border-l-red-500">
          <p className="text-gray-400 text-xs uppercase font-semibold">Inactivos</p>
      ...
      <div className="flex flex-wrap justify-between items-center mb-6 gap-4">
        ...
        <div className="flex gap-2">
          <button onClick={getInstructores} className="p-2.5 bg-[#161B22] border border-gray-800 rounded-lg hover:text-[#FACC15] transition shadow-sm">
            <RefreshCcw size={20} />
          </button>
        </div>
      </div>
      ...
      <div className="bg-[#161B22] rounded-xl border border-gray-800 overflow-hidden shadow-xl">
        <table className="w-full text-left">
          <thead className="bg-[#0B0E11] text-gray-500 text-xs uppercase border-b border-gray-800">
            <tr>
              <th className="p-4 cursor-pointer hover:text-white transition" onClick={() => handleSort('nombre_completo')}>
                Instructor <ArrowUpDown size={12} className="inline ml-1 opacity-50" />
              </th>
              <th className="p-4 cursor-pointer hover:text-white transition" onClick={() => handleSort('especialidad')}>
                Especialidad <ArrowUpDown size={12} className="inline ml-1 opacity-50" />
              </th>
          ...
          <tbody className="divide-y divide-gray-800 text-sm">
            {filteredInstructors.map((ins) => (
              <tr key={ins.id_instructor} className="hover:bg-gray-800/30 transition">
                <td className="p-4 font-medium">{ins.nombre_completo}</td>
                <td className="p-4 text-gray-400">{ins.especialidad}</td>
                <td className="p-4 text-gray-400 italic">{ins.contacto || 'Sin dato'}</td>
                <td className="p-4">
                  <span className={`px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider ${
                    ins.estatus === 'Activo' ? 'bg-green-500/10 text-green-400 border border-green-500/20' : 'bg-red-500/10 text-red-400 border border-red-500/20'
                  }`}>
                    {ins.estatus}
                  </span>
                </td>
                <td className="p-4 text-right">
                  <button onClick={() => openEdit(ins)} className="text-gray-400 hover:text-[#FACC15] mr-4 transition">Editar</button>
                  <button onClick={async () => { 
                    if(confirm("¿Inactivar a este instructor?")) { 
                      await axios.delete(`${API_URL}/${ins.id_instructor}`); 
                      getInstructores(); 
                    } 
                  }} className="text-red-900 hover:text-red-500 transition">Baja</button>
                </td>
              </tr>
            ))}
          </tbody>
        ...
        <div className="fixed inset-0 bg-black/80 flex justify-center items-center backdrop-blur-sm z-50 p-4">
          <div className="bg-[#161B22] p-8 rounded-2xl border border-gray-800 w-full max-w-md shadow-2xl animate-in fade-in zoom-in duration-200">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="text-xs text-gray-500 mb-1 block uppercase">Nombre Completo</label>
                <input 
                  type="text" required
              ...
              <div>
                <label className="text-xs text-gray-500 mb-1 block uppercase">Especialidad</label>
                <input 
                  type="text" required
                  className="w-full bg-[#0B0E11] border border-gray-800 p-3 rounded-lg focus:border-[#FACC15] outline-none transition"
              ...
              <div>
                <label className="text-xs text-gray-500 mb-1 block uppercase">Contacto (Tel/Email)</label>
                <input 
                  type="text"
                  className="w-full bg-[#0B0E11] border border-gray-800 p-3 rounded-lg focus:border-[#FACC15] outline-none transition"
              ...
              {editMode && (
                <div>
                  <label className="text-xs text-gray-500 mb-1 block uppercase">Estatus</label>
                  <select 
                    className="w-full bg-[#0B0E11] border border-gray-800 p-3 rounded-lg focus:border-[#FACC15] outline-none transition"
              ...
              <div className="flex justify-end gap-3 pt-6">
                <button type="button" onClick={() => setModalOpen(false)} className="text-gray-500 hover:text-white px-4 transition">Cerrar</button>
                <button type="submit" className="bg-[#FACC15] text-black px-8 py-3 rounded-xl font-bold hover:brightness-110 active:scale-95 transition">
