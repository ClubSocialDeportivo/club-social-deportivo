...
const Sesiones = () => {
  ...
  const sesionesDelDia = useMemo(
    () => agenda.filter(s => s.fecha === selectedYMD)
         .sort((a, b) => a.hora_inicio.localeCompare(b.hora_inicio)),
    [agenda, selectedYMD]
  );

  const reservasDelDia = useMemo(
    () => reservas.filter(r => r.fecha === selectedYMD)
         .sort((a, b) => a.hora_inicio.localeCompare(b.hora_inicio)),
  ...
  const stats = {
    ...
    sesionesMes:    agenda.filter(s => s.fecha?.startsWith(toYMD(currentMonth).slice(0, 7))).length,
    reservasMes:    reservas.filter(r => r.fecha?.startsWith(toYMD(currentMonth).slice(0, 7))).length,
  };
  ...
  const selectedLabel = selectedDate.toLocaleDateString("es-MX", { weekday: "long", day: "numeric", month: "long" });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard title="Sesiones hoy"   value={stats.sesionesHoy}  icon={<Activity size={20}/>}  color="text-blue-400"/>
        <StatCard title="Reservas hoy"   value={stats.reservasHoy}  icon={<BookOpen size={20}/>}  color="text-purple-400"/>
        <StatCard title="Sesiones mes"   value={stats.sesionesMes}  icon={<Calendar size={20}/>}  color="text-green-400"/>
        <StatCard title="Reservas mes"   value={stats.reservasMes}  icon={<Users size={20}/>}     color="text-yellow-400"/>
      </div>
        ...
        <div className="lg:col-span-2 bg-[#14171c] rounded-xl border border-gray-800 overflow-hidden">
          ...
          <div className="grid grid-cols-7">
            {calendarDays.map((day, idx) => {
              if (!day) return <div key={`empty-${idx}`} className="h-20 border-b border-r border-gray-800/50"/>;
                    ...
                    <div className="flex gap-1 mt-1 flex-wrap">
                      {events.agenda > 0 && (
                        <span className="flex items-center gap-0.5">
                          ...
                          {events.agenda > 1 && <span className="text-[9px] text-green-400">{events.agenda}</span>}
                        </span>
                      )}
                      {events.reservas > 0 && (
                        <span className="flex items-center gap-0.5">
                          <span className="w-1.5 h-1.5 rounded-full bg-purple-400"/>
                          {events.reservas > 1 && <span className="text-[9px] text-purple-400">{events.reservas}</span>}
                        </span>
                      )}
                    </div>
                  )}
        ...
        <div className="space-y-4">
          ...
          <div className="bg-[#14171c] rounded-xl border border-gray-800 overflow-hidden">
            ...
            <div className="divide-y divide-gray-800 max-h-64 overflow-y-auto">
              {loading ? (
                <p className="text-center py-6 text-gray-500 text-sm">Cargando...</p>
              ) : sesionesDelDia.length === 0 ? (
                <p className="text-center py-6 text-gray-500 text-sm italic">Sin sesiones este día</p>
              ) : sesionesDelDia.map(s => (
                <button
                  key={s.id_sesion}
                  onClick={() => handleViewSesion(s.id_sesion)}
          ...
          <div className="bg-[#14171c] rounded-xl border border-gray-800 overflow-hidden">
            ...
            <div className="divide-y divide-gray-800 max-h-64 overflow-y-auto">
              {loading ? (
                ...
                <p className="text-center py-6 text-gray-500 text-sm italic">Sin reservas este día</p>
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">
                        {r.socio ? `${r.socio.nombre} ${r.socio.apellidos}` : `Socio #${r.id_socio}`}
                      </p>
                    </div>
                    <div className="text-right shrink-0">
                      <p className="text-xs text-gray-400 flex items-center gap-1">
...
const EstadoBadge = ({ status, type }) => {
  ...
  return (
    <span className={`mt-1 inline-block px-1.5 py-0.5 rounded text-[10px] font-bold ${style}`}>
      {status}
    </span>
  );
};
