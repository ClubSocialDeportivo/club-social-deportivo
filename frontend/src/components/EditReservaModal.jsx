import { X, Save, Calendar, Clock, MapPin, User, Hash, FileText, Edit3 } from "lucide-react";
import { useState, useEffect } from "react";

const EditReservaModal = ({ isOpen, onClose, data, onUpdate }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`http://localhost:8000/api/reservas/${data.id_reserva}`, {
        method:  "PUT",
      ...
      <div className="bg-[#1c1f26] border border-gray-800 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="p-6 border-b border-gray-800 flex justify-between items-center bg-[#23272f]">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-500 text-xs font-mono">{data.folio_reserva}</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5 max-h-[75vh] overflow-y-auto">
          ...
          <div className="space-y-1.5">
            ...
            <input
              type="text"
              className={`w-full bg-gray-900 border rounded-lg p-2.5 text-white outline-none transition-all font-mono ${errors.folio_reserva ? "border-red-500" : "border-gray-700 focus:border-blue-500"}`}
              value={formData.folio_reserva || ""}
              onChange={e => field("folio_reserva", e.target.value)}
            />
            {errors.folio_reserva && <p className="text-red-400 text-xs">{errors.folio_reserva}</p>}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <select
                className={`w-full bg-gray-900 border rounded-lg p-2.5 text-white outline-none cursor-pointer transition-all ${errors.id_socio ? "border-red-500" : "border-gray-700 focus:border-blue-500"}`}
          ...
          <div className="bg-gray-900/40 p-4 rounded-xl border border-gray-800 space-y-4">
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] text-gray-500 font-bold uppercase">Estatus</label>
              <select
          ...
          <div className="bg-gray-900/40 p-4 rounded-xl border border-gray-800 flex items-center justify-between">
            ...
            >
              <span className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-all duration-200 ${formData.estatus_noshow ? "left-7" : "left-1"}`}/>
            </button>
          </div>
