...
const AddAgendaModal = ({ isOpen, onClose, onRefresh }) => {
  ...
  // Carga catálogos cuando abre el modal
  useEffect(() => {
    if (!isOpen) return;
    setFormData({
      // ← definido directamente aquí, sin dependencia externa
      id_disciplina: "",
      id_instructor: "",
      id_espacio: "",
      fecha: "",
  ...
  const fetchCatalogos = async () => {
    try {
      const [d, i, e] = await Promise.all([
        fetch("http://localhost:8000/api/agenda/catalogo/disciplinas").then(
          (r) => r.json(),
        ),
        fetch("http://localhost:8000/api/agenda/catalogo/instructores").then(
          (r) => r.json(),
        ),
      ...
      <div className="bg-[#1c1f26] border border-gray-800 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden">
        ...
        >
          ...
          <div className="space-y-1.5">
            ...
            <select
              className={`w-full bg-gray-900 border rounded-lg p-2.5 text-white outline-none cursor-pointer transition-all ${errors.id_disciplina ? "border-red-500" : "border-gray-700 focus:border-green-500"}`}
            ...
            {errors.id_disciplina && (
              <p className="text-red-400 text-xs">{errors.id_disciplina}</p>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <select
                className={`w-full bg-gray-900 border rounded-lg p-2.5 text-white outline-none cursor-pointer transition-all ${errors.id_instructor ? "border-red-500" : "border-gray-700 focus:border-green-500"}`}
              ...
              {errors.id_instructor && (
                <p className="text-red-400 text-xs">{errors.id_instructor}</p>
              )}
            </div>
            <div className="space-y-1.5">
              <label className="text-xs text-gray-500 font-bold uppercase flex items-center gap-1">
              ...
              <select
                className={`w-full bg-gray-900 border rounded-lg p-2.5 text-white outline-none cursor-pointer transition-all ${errors.id_espacio ? "border-red-500" : "border-gray-700 focus:border-green-500"}`}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <input
                type="date"
                className={`w-full bg-gray-900 border rounded-lg p-2.5 text-white outline-none transition-all ${errors.fecha ? "border-red-500" : "border-gray-700 focus:border-green-500"}`}
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                min={1}
                placeholder="Ej. 20"
                className="w-full bg-gray-900 border border-gray-700 focus:border-green-500 rounded-lg p-2.5 text-white outline-none transition-all"
          ...
          <div className="bg-gray-900/40 p-4 rounded-xl border border-gray-800 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <input
                  type="time"
                  className={`w-full bg-gray-900 border rounded-lg p-2 text-white outline-none transition-all ${errors.hora_inicio ? "border-red-500" : "border-gray-700 focus:border-green-500"}`}
              ...
              <div className="space-y-1.5">
                ...
                <input
                  type="time"
                  className={`w-full bg-gray-900 border rounded-lg p-2 text-white outline-none transition-all ${errors.hora_fin ? "border-red-500" : "border-gray-700 focus:border-green-500"}`}
            ...
            <div className="space-y-1.5">
              ...
              <select
                className="w-full bg-gray-900 border border-gray-700 focus:border-green-500 rounded-lg p-2 text-white outline-none cursor-pointer"
          ...
          <div className="pt-2 flex gap-3">
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex-1 bg-green-600 hover:bg-green-700 disabled:opacity-50 text-white font-bold py-3 rounded-xl flex items-center justify-center transition-all shadow-lg shadow-green-900/20"
            >
              {loading ? (
                <span className="animate-pulse">Guardando...</span>
              ) : (
                <>
                  <Save size={18} className="mr-2" /> Crear Sesión
                </>
              )}
            </button>
          </div>
